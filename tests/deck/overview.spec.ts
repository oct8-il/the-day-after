...
test.describe('the source chips', () => {
  test('a chip names its type on the phone, where the desktop draws a dot alone', async ({ page }) => {
    ...
    const chips = await page.evaluate(() =>
      [...document.querySelectorAll('.deck-ov-body a.chip')].map((a) => ({
  ...
  test('a span resting on two claims says +1', async ({ page }) => {
    ...
    const ns = await page.evaluate(() =>
      [...document.querySelectorAll('.deck-ov-body a.chip .chip-n')].map((e) => e.textContent));
    expect(ns).toEqual(['+1']);
  });
  ...
  test('the chip carries its type colour, and every type is distinguishable', async ({ page }) => {
    ...
    const colours = await page.evaluate(() =>
      [...document.querySelectorAll('.deck-ov-body a.chip .chip-dot')]
...
test.describe('the sources carousel', () => {
  test('212px cards, outlet, a two-line quote and a date', async ({ page }) => {
    ...
    const card = await page.evaluate(() => {
      const li = document.querySelector('.deck-ov-card')!;
  ...
  test('one card per cited claim, in the order the chips are met', async ({ page }) => {
    ...
    const order = await page.evaluate(() => ({
      cards: [...document.querySelectorAll('.deck-ov-card')].map((c) => c.getAttribute('data-claim')),
      chips: [...document.querySelectorAll('.deck-ov-body a.chip')].map((a) => a.getAttribute('href')?.slice(1)),
    }));
    // Every chip's first claim has a card, and the first card is the first chip's.
    expect(order.cards[0]).toBe(order.chips[0]);
    for (const href of order.chips) expect(order.cards).toContain(href);
  });
...
test.describe('a chip moves the carousel', () => {
  test('it lands its source card and marks it, without leaving the deck', async ({ page }) => {
    await open(page, 't01');
    ...
    const landed = await page.evaluate((id) => {
      ...
      const card = rail.querySelector(`[data-claim="${id}"]`)!;
      return {
        marked: card.hasAttribute('data-on'),
        marks: rail.querySelectorAll('[data-on]').length,
        dx: card.getBoundingClientRect().left - rail.getBoundingClientRect().left,
      };
    }, href);

    expect(landed.marked).toBe(true);
    expect(landed.marks).toBe(1);
    expect(Math.abs(landed.dx)).toBeLessThan(2);
    // The deck stays where it is: no navigation, and the slide does not move.
    expect(page.url()).toBe(before);
    expect(await page.evaluate(() => document.querySelector('.deck')!.getAttribute('data-at'))).toBe('1');
  });

  test('a +N chip lands the first of its claims, and the second is adjacent', async ({ page }) => {
    await open(page, 't01');
    const ids = await page.evaluate(() => {
      const a = [...document.querySelectorAll<HTMLAnchorElement>('.deck-ov-body a.chip')]
        .find((x) => x.querySelector('.chip-n'))!;
      a.click();
      return [...document.querySelectorAll('.deck-ov-card')].map((c) => c.getAttribute('data-claim'));
    });
    await page.waitForTimeout(600);

    const landedId = await page.evaluate(() =>
      document.querySelector('.deck-ov-card[data-on]')!.getAttribute('data-claim'));
    const at = ids.indexOf(landedId);
    expect(at).toBeGreaterThanOrEqual(0);
    // Deduplicated in order of first appearance, so the pair is side by side.
    expect(ids[at + 1]).toBeTruthy();
  });
});
...
test.describe('the floor', () => {
  test('one claim and one span still make a screen', async ({ page }) => {
    ...
    const m = await page.evaluate(() => ({
      chips: document.querySelectorAll('.deck-ov-body a.chip').length,
      cards: document.querySelectorAll('.deck-ov-card').length,
